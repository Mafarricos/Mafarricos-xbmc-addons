...
addon_id = 'plugin.video.tabonito'
selfAddon = xbmcaddon.Addon(id=addon_id)
addonfolder = selfAddon.getAddonInfo('path')
artfolder = '/resources/img/'
siteurl = 'http://www.tabonito.pt'
...
def CATEGORIES():
	try:
		...
		addDir('[COLOR blue]TÁ BONITO[/COLOR]',siteurl,'',addonfolder+'/icon.png',False,1,'','')
...
def listar_categorias(url):
	...
	entry = re.findall('</span>          <h2 class="block-title">Categorias</h2>.+?</tbody>',conteudo,re.DOTALL)
	for entries in entry:
		link = re.compile('<div class="item"><a href="(.+?)">(.+?)</a>').findall(entries)
		tamanho = len(link)
		for url,name in link:
			if not 'Imagens' in name: addDir(name,siteurl+url,1,addonfolder+'/icon.png',True,tamanho,'','')		

def listar_videos(url):
	addDir('[COLOR yellow]Inicio[/COLOR]','',0,'',True,1,'','')	
	conteudo = abrir_url(url)	
	entry = re.findall('<span class="field-content">Vídeos / </span>  </span>.+?<div class="views-field views-field-php-1">',conteudo,re.DOTALL)
	tamanho = len(entry)
	for entries in entry:	
		data = re.compile('<span class="field-content">(.+?)</span>  </span>\s+<div>        <h2 class="title">').findall(entries)
		titulo = re.compile('<h2 class="title"><a href=".+?">(.+?)</a></h2>').findall(entries)
		print '##titulo '+titulo[0]
		plot = re.compile('<div class="field-content"><p>(.+?)</p>').findall(entries)
		url = re.compile('<iframe.+?src="(.+?)"').findall(entries)
		urlfinal = ''
		img = ''
		size = ''
		if url:
			if 'soundcloud' in url[0]: urlfinal = 'KT'
			if 'youtube' in url[0]:
			...
			if 'sapo' in url[0]:
				...
				for dominio,ID in url2: 
					...
					try:
						...
						for horas,minutos,segundos in duration: size = (int(segundos))+(int(minutos)*60)+(int(horas)*3600)
						imagem = re.compile('<meta property="og:image" content="(.+?)"/>').findall(sapoAPI)
						if imagem: img = imagem[0]		
					except:
						urlfianl='KT'
						pass
			if 'dailymotion' in url[0]:
			...
			if 'videolog' in url[0]:
				try:
					...
					pass						
			if urlfinal <> 'KT':
				try: addDir('[COLOR green]'+title+'[/COLOR]',urlfinal,3,img,False,tamanho,size,informacao)
				except: pass
	link = re.compile('<li class="pager-current">.+?</li><li class="pager-item"><a title="Ir para a página \d+" href="(.+?)">\d+</a></li><li class="pager-item">').findall(conteudo)
	if not link: link = re.compile('<a title="Ir para a página \d+" href="(.+?)">\d+</a></li><li class="pager-item">').findall(conteudo)
	if link:
		link[0] = link[0].replace('&amp;','&')
		link[0] = link[0].replace('%20',' ')		
		addDir("[COLOR yellow]Próxima >>[/COLOR]",siteurl+link[0],1,'',True,1,size,'')		
